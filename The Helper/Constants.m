//
//  Constants.m
//  The Helper
//
//  Created by Nidal on 10/4/12.
//
//

#import "Constants.h"

@implementation Constants
NSString *emiNegativeMessage = @"VALUES are NEGATIVE";
NSString *emiOutOfBoundMessage = @"RATE CANT BE GREATER THAN 100";
NSString *emiInvalidTypeMessage = @"Invalid Type";
NSString *emiFieldEmptyMessage = @"VALUES NOT ENTERED";
NSString *emiAlertButton = @"Alert";
NSString *emiSegueIdentifier = @"Result";
NSString *emiOkayButton = @"Okay";
NSString *negativeMessage = @"VALUES are NEGATIVE";
NSString *outOfBoundMessage = @"RATE CANT BE GREATER THAN 100";
NSString *fieldEmptyMessage = @"VALUES NOT ENTERED";
NSString *alertButton = @"Alert";
NSString *segueIdentifier = @"Result";
NSString *okayButton = @"Okay";
NSString *tipServerUrl = @"http://localhost:8888/PhpProject1/TipServer.php";
NSString *emiServerUrl = @"http://localhost:8888/PhpProject1/EmiServer.php";

@end
